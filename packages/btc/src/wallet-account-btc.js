...
// @tetherto/wdk-wallet-btc@1.0.0-beta.9 verbatim. Modifications:
//   - signTransaction / sendTransaction / _buildSignedTransaction /
//     _getRawTransaction accept an optional `memo` argument
//   - when `memo` is supplied, an OP_RETURN output carrying the memo
//     bytes is appended to the PSBT (value 0); rejected with
//     RangeError if the payload exceeds the 80-byte standardness cap
//   - module-level _normalizeMemo() helper near the top of the file
// Everything else (key derivation, transports, signing, broadcast,
// UTXO polling) is upstream.
 ...
 *
 * @internal Exported only for unit tests; not part of the package's
 *           public surface (not re-exported from index.js).
...
export default class WalletAccountBtc extends WalletAccountReadOnlyBtc {
  ...
  constructor (seed, path, config = {}) {
    ...
    /**
     * Whether this account has been disposed. Once true, every public
     * method short-circuits via _assertNotDisposed() and dispose()
     * itself becomes a no-op (idempotent).
     *
     * @private
     * @type {boolean}
     */
   ...
   * calls fail loudly instead of hitting `sodium_memzero(null)` or
   * returning a stale read-only view.
   *
   * @private
   */
  _assertNotDisposed () {
  ...
  async transfer (options) {
    // Assert before the "not supported" throw so post-dispose callers
    // get the canonical "Cannot use disposed wallet account" message
    // matching every other key-touching method on this class.
    this._assertNotDisposed()
   ...
   * and closing the connection with the server. Idempotent — a second
   * call is a no-op. Before this guard the second call would hit
   * `sodium_memzero(undefined)` on the cleared masterNode.
   */
  dispose () {
    if (this._disposed) {
      return
  ...
  async _getRawTransaction ({ utxos, to, value, fee, feeRate, changeValue, memo }) {
    ...
    const buildAndSign = async (rcptVal, chgVal) => {
      ...
      if (opReturnScript) psbt.addOutput({ script: opReturnScript, value: 0 })
